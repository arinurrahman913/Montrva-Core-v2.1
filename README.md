...
`dashboard/data/` bukan berkas konfigurasi kecil: `historical_timeline.json` ~598 MB, `evidence.json` ~250 MB, `personal_history.json` ~188 MB. Total ~970 MB JSON, yang kalau di-`json.load` semua mengembang jadi **~3,3 GB objek Python** (~3,5×).
...
- **`historical_timeline.json` tumbuh tanpa rem yang berarti**: 14 hari = 598 MB, `RETENTION_DAYS=730` di `historical.py` memproyeksikan **~31 GB** dan pemangkasan pertama baru Juli 2028. 97,1% tiap entry adalah `aggregator_output`, yang justru dibuang endpoint ringkasannya. Rencana (belum dikerjakan): snapshot penuh hanya untuk entry terakhir, sisanya tipis, horizon ikut turun. **Menurunkan `RETENTION_DAYS` saja tidak cukup** — ekor tipis 730 hari × 4.233 ticker sendirian sudah ~592 MB.
